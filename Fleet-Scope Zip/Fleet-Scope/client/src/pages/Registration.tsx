...
import { RegConversations } from "@/components/RegConversations";
import {
  Table,
  TableBody,
  TableCell,
...
export default function Registration() {
  ...
  const { data: pmfStickersData } = useQuery<{ success: boolean; assetIds: string[] }>({
    queryKey: ["/api/pmf/registration-stickers-needed"],
  });
  ...
  const importMutation = useMutation({
    mutationFn: async (data: string) => {
      const response = await apiRequest("POST", "/api/registration/import", { data });
  ...
  const trackingMutation = useMutation({
    ...
    onMutate: async (variables) => {
      await queryClient.cancelQueries({ queryKey: ["/api/registration"] });
      const previousData = queryClient.getQueryData(["/api/registration"]);
      queryClient.setQueryData(["/api/registration"], (old: any) => {
    ...
    onError: (error: Error, _variables, context) => {
      if (context?.previousData) {
        queryClient.setQueryData(["/api/registration"], context.previousData);
      }
      toast({
        title: "Update Failed",
        description: error.message,
