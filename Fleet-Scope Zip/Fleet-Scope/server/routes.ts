...
import { db } from "./db";
import { approvedCostRecords, vehicleMaintenanceCosts, pmfRows, spareVehicleDetails, registrationTracking, rentalWeeklyManual, pickupWeeklySnapshots, regMessages, regScheduledMessages } from "@shared/schema";
import { sql, eq, desc, and, isNull, count } from "drizzle-orm";
import { broadcastMessage, getNextAllowedSendTime, sendTwilioMessage } from "./reg-messaging";
import { insertTruckSchema, updateTruckSchema, insertTrackingRecordSchema, parseStatus, validateStatus, normalizeStatusLegacy } from "@shared/schema";
import { z } from "zod";
import { testConnection, executeQuery, getTableData, getTableSchema } from "./snowflake";
import { trackPackage, testUPSConnection, checkRateLimit } from "./ups";
import { parqApi } from "./pmf-api";
import { fetchFleetFinderData, fetchFleetFinderVehicleInfo, prewarmFleetFinderCache, type FleetFinderLocationData, type FleetFinderVehicleInfo } from "./fleet-finder";
import { fetchSamsaraLocations, testSamsaraConnection, type SamsaraLocationData } from "./samsara";
import { reverseGeocode, batchReverseGeocode, getGeocodeStats } from "./reverse-geocode";
...
async function autoCaptureByovSnapshot(): Promise<void> {
  try {
    ...
    const existingSnapshots = await storage.getByovWeeklySnapshots(1);
    ...
    await storage.createByovWeeklySnapshot({
      totalEnrolled,
      assignedInFleet: 0,
...
async function autoCaptureFleetSnapshot(): Promise<void> {
  try {
    ...
    const existingSnapshots = await storage.getFleetWeeklySnapshots(1);
    if (existingSnapshots.length > 0 && 
        existingSnapshots[0].weekNumber === weekNumber && 
        existingSnapshots[0].weekYear === weekYear) {
      return; // Already captured this week
    ...
    const pmfDataset = await storage.getPmfDataset();
    const pmfCount = pmfDataset.rows?.length || 0;
    
    await storage.createFleetWeeklySnapshot({
      totalFleet,
      assignedCount,
      unassignedCount,
      pmfCount,
...
async function autoCapturePmfStatusSnapshot(): Promise<void> {
  try {
    ...
    const existingSnapshots = await storage.getPmfStatusWeeklySnapshots(1);
    if (existingSnapshots.length > 0 && 
        existingSnapshots[0].weekNumber === weekNumber && 
        existingSnapshots[0].weekYear === weekYear) {
      return; // Already captured this week
    ...
    await storage.createPmfStatusWeeklySnapshot({
...
async function autoCaptureRepairSnapshot(): Promise<void> {
  try {
    ...
    const existingSnapshots = await storage.getRepairWeeklySnapshots(1);
    if (existingSnapshots.length > 0 && 
        existingSnapshots[0].weekNumber === weekNumber && 
        existingSnapshots[0].weekYear === weekYear) {
      return; // Already captured this week
    ...
    await storage.createRepairWeeklySnapshot({
      totalInRepair,
      activeRepairs,
      completedThisWeek,
      capturedBy: "Auto",
...
async function autoSyncActivityLogs(): Promise<void> {
  try {
    ...
    const pmfData = await storage.getPmfDataset();
    ...
    await storage.clearPmfActivityLogs();
    ...
    await storage.upsertPmfActivityLogs(allLogs);
    await storage.updatePmfActivitySyncMeta(vehicleMap.length, totalLogsFetched, 'success');
    ...
    await storage.updatePmfActivitySyncMeta(0, 0, 'failed', error.message);
  }
}
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/trucks", async (req, res) => {
    try {
      ...
      const allTrackingRecords = await storage.getTrackingRecords();
  ...
  app.get("/api/pickups-scheduled-this-week", async (req, res) => {
    try {
      const now = new Date();
      ...
      if (existing.rows && existing.rows.length > 0) {
        ...
        await db.insert(pickupWeeklySnapshots).values({
          weekNumber,
          weekYear,
  ...
  app.get("/api/pickup-weekly-snapshots", async (req, res) => {
    try {
  ...
  app.patch("/api/pickup-weekly-snapshots/:id", async (req, res) => {
    try {
      const { id } = req.params;
  ...
  app.get("/api/public/rentals", async (req, res) => {
    try {
      const trucks = await storage.getAllTrucks();
  ...
  app.get("/api/public/rentals/summary", async (req, res) => {
  ...
  app.get("/api/public/rentals/:truckNumber", async (req, res) => {
    try {
  ...
  app.get("/api/public/registrations", async (req, res) => {
    try {
  ...
  app.get("/api/public/registrations/:truckNumber", async (req, res) => {
    try {
      const { truckNumber } = req.params;
  ...
  app.post("/api/public/spares/:vehicleNumber", async (req, res) => {
    try {
      ...
      // Save to PostgreSQL
      await storage.upsertSpareVehicleDetail(normalizedVehicleNumber, pgUpdates);
  ...
  // Requires X-API-Key header for authentication
  app.get("/api/public/spares", async (req, res) => {
    try {
      // API Key authentication
      const apiKey = req.headers['x-api-key'];
      const expectedApiKey = process.env.PUBLIC_SPARES_API_KEY;
      ...
      const allDetails = await storage.getAllSpareVehicleDetails();
  ...
  // Requires X-API-Key header for authentication
  app.get("/api/public/spares/:vehicleNumber", async (req, res) => {
    try {
      // API Key authentication
      const apiKey = req.headers['x-api-key'];
      const expectedApiKey = process.env.PUBLIC_SPARES_API_KEY;
      ...
      // Get from PostgreSQL
      const detail = await storage.getSpareVehicleDetail(normalizedVehicleNumber);
  ...
  app.get("/api/public/all-vehicles", requirePublicApiKey, async (_req, res) => {
    try {
  ...
  app.get("/api/public/pmf", requirePublicApiKey, async (_req, res) => {
    try {
      const dataset = await storage.getPmfDataset();
      if (!dataset.import) {
  ...
  app.get("/api/public/pos", requirePublicApiKey, async (_req, res) => {
    try {
  ...
  app.get("/api/public/po-priority", requirePublicApiKey, async (_req, res) => {
    try {
      const port = process.env.PORT || 5000;
  ...
  app.get("/api/public/decommissioning", requirePublicApiKey, async (_req, res) => {
    try {
      const vehicles = await storage.getAllDecommissioningVehicles();
      const allTrucks = await storage.getAllTrucks();
  ...
  app.get("/api/public/fleet-cost", requirePublicApiKey, async (_req, res) => {
    try {
      const port = process.env.PORT || 5000;
  ...
  app.get("/api/public/executive-summary", requirePublicApiKey, async (_req, res) => {
    try {
      const allTrucks = await storage.getAllTrucks();
  ...
  app.get("/api/public/metrics", requirePublicApiKey, async (req, res) => {
    try {
      const allTrucks = await storage.getAllTrucks();
      const current = {
      ...
      const historical = await storage.getMetricsSnapshots(startDate, endDate);
  ...
  app.get("/api/public/action-tracker", requirePublicApiKey, async (_req, res) => {
    try {
      const allTrucks = await storage.getAllTrucks();
  ...
  app.get("/api/public/call-logs", requirePublicApiKey, async (_req, res) => {
    try {
      const logs = await storage.getRecentCallLogs(200);
  ...
  app.get("/api/public/follow-ups", requirePublicApiKey, async (_req, res) => {
    try {
  ...
  app.get("/api/tech-specialty", async (req, res) => {
    try {
  ...
  app.post("/api/tech-specialty/batch", async (req, res) => {
    try {
  ...
  app.get("/api/trucks/po-status", async (req, res) => {
    try {
      const now = Date.now();
  ...
  app.get("/api/trucks/scraper-status", async (_req, res) => {
    try {
  ...
  app.get("/api/trucks/scraper-detail/:truckNumber", async (req, res) => {
    try {
  ...
  app.post("/api/trucks/:id/call-repair-shop", async (req, res) => {
    try {
      ...
      if (!response.ok) {
        ...
        try {
          await storage.updateTruck(truck.id, {
  ...
  app.post("/api/trucks/:id/call-technician", async (req, res) => {
    try {
      if (!process.env.ELEVENLABS_API_KEY) {
        return res.status(500).json({ message: "ElevenLabs API key not configured" });
      }
  ...
  app.post("/api/batch-call/start", async (req, res) => {
    try {
          ...
          const promises = batch.map(async (truckId: string) => {
            try {
              ...
              if (!response.ok) {
                ...
                if (callType === "tech") {
                  await storage.updateTruck(truck.id, { lastTechCallDate: new Date() as any, lastTechCallStatus: "Call Failed", lastTechCallSummary: `Batch call API error` });
                } else {
                  await storage.updateTruck(truck.id, { lastCallDate: new Date() as any, lastCallStatus: "Call Failed", lastCallSummary: `Batch call API error` });
                }
              ...
              if (callType === "tech") {
                await storage.updateTruck(truck.id, { lastTechCallDate: new Date() as any, lastTechCallConversationId: conversationId, lastTechCallSummary: null as any });
              } else {
                await storage.updateTruck(truck.id, { lastCallDate: new Date() as any, lastCallConversationId: conversationId, lastCallSummary: null as any });
              }
  ...
  app.get("/api/batch-call/status/:batchId", async (req, res) => {
  ...
  app.post("/api/batch-call/cancel/:batchId", async (req, res) => {
    const job = batchJobs.get(req.params.batchId);
    if (!job) return res.status(404).json({ message: "Batch not found" });
    job.cancelled = true;
  ...
  app.get("/api/call-logs/:truckId", async (req, res) => {
    try {
      const logs = await storage.getCallLogsByTruckId(req.params.truckId);
      res.json(logs);
    } catch (error: any) {
      res.status(500).json({ message: error.message });
    }
...
Respond ONLY with valid JSON, no other text.`;
      ...
      if (callType === "tech") {
        await storage.updateTruck(truck.id, { lastTechCallSummary: summary, lastTechCallStatus: status });
      } else {
        await storage.updateTruck(truck.id, { lastCallSummary: summary, lastCallStatus: status });
      }
      ...
      try {
        ...
        if (callLog) {
          ...
          if (mappedOutcome === "VEHICLE_READY" && callLog.callType === "shop") {
            const truckForAutoCall = await storage.getTruck(callLog.truckId);
            if (truckForAutoCall?.techPhone && truckForAutoCall.techPhone.trim()) {
              ...
              try {
                ...
                if (techResponse.ok) {
                  ...
                  await storage.updateTruck(truckForAutoCall.id, {
  ...
  app.get("/api/trucks/:id/actions", async (req, res) => {
    try {
      const actions = await storage.getTruckActions(req.params.id);
      res.json(actions);
    } catch (error: any) {
  ...
  app.post("/api/trucks", async (req, res) => {
    try {
      ...
      const existingTruck = await storage.getTruckByNumber(validated.truckNumber);
      if (existingTruck) {
      ...
      const truck = await storage.createTruck(validated);
  ...
  const updateTruckHandler = async (req: any, res: any) => {
    try {
      const existing = await storage.getTruck(req.params.id);
      if (!existing) {
        return res.status(404).json({ message: "Truck not found" });
      }
      ...
      const truck = await storage.updateTruck(req.params.id, validated);
  ...
  // PUT update truck
  app.put("/api/trucks/:id", updateTruckHandler);
  
  // PATCH update truck (alias for PUT)
  app.patch("/api/trucks/:id", updateTruckHandler);

  app.post("/api/trucks/call-import", async (req, res) => {
    try {
      const { rows } = req.body;
      ...
      for (let i = 0; i < rows.length; i++) {
        try {
          ...
          const existing = await storage.getTruckByNumber(rawTruckNumber) || await storage.getTruckByNumber(stripped);
          if (!existing) {
            results.notFound++;
          ...
          if (Object.keys(updates).length > 0) {
            await storage.updateTruck(existing.id, updates);
            results.updated++;
          }
        } catch (rowErr: any) {
  ...
  app.post("/api/trucks/bulk-import", async (req, res) => {
    try {
  ...
  app.post("/api/trucks/bulk-sync", async (req, res) => {
    try {
      ...
      const result = await storage.bulkSyncTrucks(truckNumbers, syncedBy);
  ...
  app.post("/api/trucks/consolidate", async (req, res) => {
    try {
      ...
      const result = await storage.consolidateTrucks(validatedEntries, consolidatedBy || 'System');
  ...
  app.post("/api/trucks/update-reg-expiry", async (req, res) => {
    try {
      ...
      for (const entry of entries) {
        ...
        if (truckId) {
          await storage.updateTruck(truckId, { holmanRegExpiry: regExpiry });
          updatedCount++;
        } else {
          notFoundCount++;
  ...
  app.post("/api/trucks/update-bill-paid", async (req, res) => {
    try {
      const { entries } = req.body;
      ...
      for (const [vehicleNo, latestDate] of latestDates) {
        ...
        if (truckId) {
          ...
          await storage.updateTruck(truckId, { billPaidDate: formattedDate });
          updatedCount++;
        } else {
          notFoundCount++;
          if (notFound.length < 20) {
  ...
  app.get("/api/truck-consolidations", async (req, res) => {
    try {
      const limit = req.query.limit ? parseInt(req.query.limit as string) : 20;
      const consolidations = await storage.getTruckConsolidations(limit);
  ...
  app.get("/api/snowflake/test", async (_req, res) => {
    try {
  ...
  app.get("/api/snowflake/schema", async (_req, res) => {
    try {
  ...
  app.post("/api/snowflake/sync-tech-data", async (req, res) => {
    try {
      ...
      for (const truck of allTrucks) {
        ...
        if (!snowflakeRecord) {
          if (Object.keys(updates).length > 0) {
            await storage.updateTruck(truck.id, updates);
          }
          continue;
        }
  ...
  app.post("/api/snowflake/sync-assigned-status", async (req, res) => {
    try {
      // Get all trucks from our database
      const allTrucks = await storage.getAllTrucks();
      ...
      for (const truck of allTrucks) {
        ...
        if (truck.snowflakeAssigned !== isAssigned) {
          await storage.updateTruck(truck.id, { snowflakeAssigned: isAssigned });
          updatedCount++;
  ...
  app.post("/api/snowflake/sync-tech-state", async (req, res) => {
    try {
      // Get all trucks from our database
      const allTrucks = await storage.getAllTrucks();
  ...
  app.get("/api/snowflake/unassigned-vehicles", async (req, res) => {
    try {
      const sql = `
        SELECT * 
  ...
  app.get("/api/snowflake/spare-assignment-status", async (req, res) => {
    try {
      const sql = `
        SELECT * 
  ...
  app.get("/api/spares", async (req, res) => {
    try {
      ...
      const localDetails = await storage.getSpareVehicleDetails(vehicleNumbers);
  ...
  app.get("/api/spares/:vehicleNumber/details", async (req, res) => {
    try {
      const { vehicleNumber } = req.params;
      const detail = await storage.getSpareVehicleDetail(vehicleNumber);
  ...
  app.get("/api/spares/locations", async (req, res) => {
    try {
      ...
      if (shouldRunCleanup) {
        try {
          ...
          const manualTrucks = await db.select().from(spareVehicleDetails);
          ...
          if (manualTrucks.length > 0) {
            ...
            for (const truck of manualTrucks) {
              ...
              if (isAssigned && !isUnassigned) {
                await db.delete(spareVehicleDetails)
      ...
      try {
        const allLocalDetails = await db.select().from(spareVehicleDetails);
      ...
      const repairShopTrucks = await storage.getAllTrucks();
      ...
      const pmfVehiclesData = await db.select({ assetId: pmfRows.assetId }).from(pmfRows);
      ...
      const decommissioningVehicles = await storage.getAllDecommissioningVehicles();
  ...
  app.patch("/api/spares/status", async (req, res) => {
    try {
      ...
      if (Object.keys(pgUpdates).length > 0) {
        ...
        const savedResult = await storage.upsertSpareVehicleDetail(normalizedVehicleNumber, pgUpdates);
  ...
  app.patch("/api/spares/confirmed-address", async (req, res) => {
    try {
  ...
  app.post("/api/spares/bulk-import", async (req, res) => {
    try {
      ...
      for (const row of data) {
        try {
          ...
          await storage.upsertSpareVehicleDetail(normalizedTruckNumber, updates);
  ...
  app.get("/api/spares/check-assigned/:truckNumber", async (req, res) => {
    try {
      const { truckNumber } = req.params;
  ...
  app.post("/api/spares/add-manual", async (req, res) => {
    try {
      const inputSchema = z.object({
      ...
      await storage.upsertSpareVehicleDetail(normalizedTruckNumber, pgData);
  ...
  app.get("/api/byov/technicians", async (req, res) => {
    try {
      const response = await fetch("https://byovdashboard.replit.app/api/external/technicians", {
        headers: { "Accept": "application/json" }
      });
  ...
  app.get("/api/byov-enrollment-status", async (_req, res) => {
    try {
  ...
  app.get("/api/byov/weekly-snapshots", async (req, res) => {
    try {
      const limit = parseInt(req.query.limit as string) || 12;
      const snapshots = await storage.getByovWeeklySnapshots(limit);
      res.json({ snapshots });
    } catch (error: any) {
  ...
  app.post("/api/byov/capture-snapshot", async (req, res) => {
    try {
      ...
      const snapshot = await storage.createByovWeeklySnapshot({
        totalEnrolled,
        assignedInFleet,
        notInFleet,
        capturedBy,
  ...
  app.get("/api/fleet/weekly-snapshots", async (req, res) => {
    try {
      const limit = parseInt(req.query.limit as string) || 12;
      const snapshots = await storage.getFleetWeeklySnapshots(limit);
      res.json({ snapshots });
    } catch (error: any) {
  ...
  app.get("/api/pmf-status/weekly-snapshots", async (req, res) => {
    try {
      const limit = parseInt(req.query.limit as string) || 12;
      const snapshots = await storage.getPmfStatusWeeklySnapshots(limit);
      res.json({ snapshots });
    } catch (error: any) {
  ...
  app.get("/api/repair/weekly-snapshots", async (req, res) => {
    try {
      const limit = parseInt(req.query.limit as string) || 12;
      const snapshots = await storage.getRepairWeeklySnapshots(limit);
      res.json({ snapshots });
    } catch (error: any) {
  ...
  app.get("/api/all-vehicles", async (req, res) => {
    try {
      ...
      const dashboardTrucks = await storage.getAllTrucks();
      ...
      const allPOsForSpares = await storage.getAllPurchaseOrders();
      ...
      const allTrucksForDeclined = await storage.getAllTrucks();
      ...
      const maintenanceCosts = await db.select().from(vehicleMaintenanceCosts);
  ...
  app.get("/api/samsara/locations", async (req, res) => {
    try {
  ...
  app.get("/api/ups/test", async (req, res) => {
    try {
  ...
  app.get("/api/tracking", async (req, res) => {
    try {
      ...
      const records = await storage.getTrackingRecords(truckId);
      res.json(records);
    } catch (error: any) {
  ...
  app.get("/api/trucks/:id/tracking", async (req, res) => {
    try {
      const records = await storage.getTrackingRecords(req.params.id);
      res.json(records);
    } catch (error: any) {
  ...
  app.post("/api/tracking", async (req, res) => {
    try {
      ...
      const existing = await storage.getTrackingRecordByNumber(validatedData.trackingNumber);
      if (existing) {
  ...
  app.post("/api/tracking/:id/refresh", async (req, res) => {
    try {
      const record = await storage.getTrackingRecord(req.params.id);
      if (!record) {
      ...
      try {
        ...
        const updatedRecord = await storage.updateTrackingRecord(record.id, {
  ...
  app.get("/api/ups/track/:trackingNumber", async (req, res) => {
    try {
  ...
  app.delete("/api/tracking/:id", async (req, res) => {
    try {
      await storage.deleteTrackingRecord(req.params.id);
  ...
  app.post("/api/tracking/refresh-all", async (req, res) => {
    try {
  ...
  app.get("/api/pmf/summary", async (req, res) => {
    try {
      const dataset = await storage.getPmfDataset();
  ...
  app.get("/api/pmf/status-events", async (req, res) => {
    try {
      ...
      const events = await storage.getPmfStatusEvents(start, end);
      res.json(events);
    } catch (error: any) {
  ...
  app.post("/api/pmf/status-events/backfill", async (req, res) => {
    try {
      const result = await storage.backfillPmfStatusEvents();
      res.json({
        success: true,
  ...
  app.get("/api/pmf/days-in-status", async (req, res) => {
    try {
  ...
  app.get("/api/pmf/registration-stickers-needed", async (req, res) => {
    try {
      const result = await db.execute(sql`
  ...
  app.post("/api/pmf/import", async (req, res) => {
    try {
      const importSchema = z.object({
      ...
      const newImport = await storage.replacePmfData({
  ...
  app.get("/api/pmf/parq/test", async (req, res) => {
    try {
  ...
  app.get("/api/pmf/parq/vehicles", async (req, res) => {
    try {
  ...
  app.get("/api/pmf/parq/statuses", async (req, res) => {
    try {
  ...
  app.get("/api/pmf/parq/lots", async (req, res) => {
    try {
  ...
  app.get("/api/pmf/parq/sync", async (req, res) => {
    try {
      const data = await parqApi.fetchAllPmfData();
      res.json({ success: true, ...data });
    } catch (error: any) {
  ...
  app.post("/api/pmf/parq/sync", async (req, res) => {
    try {
  ...
  app.get("/api/pmf/activity-logs/:assetId", async (req, res) => {
    try {
      const { assetId } = req.params;
      const logs = await storage.getPmfActivityLogs(assetId);
  ...
  app.get("/api/pmf/checkin/:vehicleId", async (req, res) => {
    try {
  ...
  app.get("/api/pmf/conditionreport/:vehicleId", async (req, res) => {
    try {
      const { vehicleId } = req.params;
  ...
  app.get("/api/pmf/tool-audit/bulk-export", async (req, res) => {
    try {
  ...
  app.get("/api/pmf/activity-sync-meta", async (req, res) => {
    try {
      const meta = await storage.getPmfActivitySyncMeta();
  ...
  app.post("/api/pmf/sync-activity-logs", async (req, res) => {
    try {
      ...
      // Insert all logs
      const result = await storage.upsertPmfActivityLogs(allLogs);
      
      // Update sync metadata
      const syncMeta = await storage.updatePmfActivitySyncMeta(
        vehicleMap.length,
        totalLogsFetched,
        'success'
      );
  ...
  app.post("/api/metrics/capture", async (req, res) => {
    try {
      const capturedBy = req.body.capturedBy || "System";
      const snapshot = await storage.captureMetricsSnapshot(capturedBy);
  ...
  app.get("/api/metrics", async (req, res) => {
    try {
      const startDate = req.query.startDate as string | undefined;
      const endDate = req.query.endDate as string | undefined;
      const snapshots = await storage.getMetricsSnapshots(startDate, endDate);
      res.json(snapshots);
    } catch (error: any) {
  ...
  app.get("/api/metrics/weekly", async (req, res) => {
    try {
      ...
      const snapshots = await storage.getMetricsSnapshots(
  ...
  app.get("/api/po-priority", async (req, res) => {
    try {
      const now = Date.now();
  ...
  app.get("/api/pos", async (req, res) => {
    try {
      const orders = await storage.getAllPurchaseOrders();
      const meta = await storage.getPoImportMeta();
  ...
  app.post("/api/pos/import", async (req, res) => {
    try {
      ...
      const existingOrders = await storage.getAllPurchaseOrders();
      ...
      await storage.clearAllPurchaseOrders();
      ...
      const created = await storage.insertPurchaseOrdersWithApproval(ordersToInsert);
      
      // Update import metadata
      await storage.updatePoImportMeta(headers || [], created, importedBy);
  ...
  app.patch("/api/pos/:id/final-approval", async (req, res) => {
    try {
      const { id } = req.params;
      ...
      await storage.updatePurchaseOrderFinalApproval(id, finalApproval);
          ...
          if (po && po.rawData) {
            ...
            if (vehicleNo) {
              ...
              if (matchingTruck && matchingTruck.mainStatus !== 'Declined Repair' && matchingTruck.mainStatus !== 'Approved for sale' && matchingTruck.mainStatus !== 'Truck Swap') {
                await storage.updateTruck(matchingTruck.id, { 
              ...
              const existingDecom = await storage.getDecommissioningVehicle(paddedTruckNumber);
              if (!existingDecom) {
                await storage.upsertDecommissioningVehicle({
  ...
  app.patch("/api/pos/:id/submitted-in-holman", async (req, res) => {
    try {
      const { id } = req.params;
      ...
      await storage.updatePurchaseOrderSubmittedInHolman(id, submittedInHolman);
  ...
  app.get("/api/pos/final-approval-options", async (req, res) => {
    try {
      const options = await storage.getUniqueFinalApprovalValues();
      res.json(options);
    } catch (error: any) {
  ...
  app.post("/api/pos/sync-declined-repairs", async (req, res) => {
    try {
      const allPOs = await storage.getAllPurchaseOrders();
      const allTrucks = await storage.getAllTrucks();
  ...
  app.get("/api/v1/external/vehicles", async (req, res) => {
    try {
      const allTrucks = await storage.getAllTrucks();
  ...
  app.get("/api/v1/external/repairs", async (req, res) => {
    try {
  ...
  app.post("/api/rentals/reconcile", async (req, res) => {
    try {
      ...
      const result = await storage.reconcileRentalList(cleanedNumbers, importedBy);
  ...
  app.get("/api/rentals/imports", async (req, res) => {
    try {
      const limit = req.query.limit ? parseInt(req.query.limit as string) : undefined;
      const imports = await storage.getRentalImports(limit);
      res.json(imports);
    } catch (error: any) {
  ...
  // Get archived trucks
  app.get("/api/rentals/archived", async (req, res) => {
    try {
      const archivedTrucks = await storage.getArchivedTrucks();
  ...
  app.get("/api/rentals/weekly-stats", async (req, res) => {
    try {
      const manualEntries = await db.select().from(rentalWeeklyManual).orderBy(sql`week_year DESC, week_number DESC`).limit(12);
  ...
  app.post("/api/rentals/weekly-manual", async (req, res) => {
    try {
  ...
  app.get("/api/rentals/summary", async (req, res) => {
    try {
      const allTrucks = await storage.getAllTrucks();
      const rentalTrucks = allTrucks.filter(t => t.mainStatus === "NLWC - Return Rental");
  ...
  app.get("/api/fleet-cost/records", async (req, res) => {
    try {
      const records = await storage.getAllFleetCostRecords();
      const meta = await storage.getFleetCostImportMeta();
      res.json({ 
        records, 
        meta,
  ...
  app.get("/api/fleet-cost/meta", async (req, res) => {
    try {
      const meta = await storage.getFleetCostImportMeta();
      res.json({ meta });
    } catch (error: any) {
  ...
  app.post("/api/fleet-cost/upload-file", upload.single('file'), async (req, res) => {
    try {
      if (!req.file) {
  ...
  app.get("/api/fleet-cost/job/:jobId", async (req, res) => {
    try {
  ...
  app.post("/api/fleet-cost/upload-chunk", async (req, res) => {
    try {
      ...
      const result = await storage.upsertFleetCostRecords(recordsToUpsert);

      // Update meta only on last chunk
      if (chunkIndex === totalChunks - 1) {
        const allRecords = await storage.getAllFleetCostRecords();
        await storage.updateFleetCostImportMeta(headers, keyColumn!, allRecords.length, importedBy);
  ...
  app.post("/api/fleet-cost/upload", async (req, res) => {
    try {
      ...
      const result = await storage.upsertFleetCostRecords(recordsToUpsert);
      await storage.updateFleetCostImportMeta(headers, 'COMPOSITE', recordsToUpsert.length, importedBy);
  ...
  app.post("/api/maintenance-costs/import", async (req, res) => {
    try {
      const { data } = req.body;
      ...
      for (const record of records) {
        await db.insert(vehicleMaintenanceCosts)
          .values({
  ...
  app.get("/api/maintenance-costs", async (req, res) => {
    try {
      const costs = await db.select().from(vehicleMaintenanceCosts);
      res.json({ 
        success: true, 
        count: costs.length, 
        data: costs 
  ...
  app.get("/api/fleet-cost/analytics", async (req, res) => {
    try {
      ...
      const [results, weeklyVehiclesResults, monthlyVehiclesResults] = await Promise.all([
        db.execute(analyticsQuery),
        db.execute(uniqueVehiclesWeeklyQuery),
        db.execute(uniqueVehiclesMonthlyQuery)
      ]);
  ...
  app.get("/api/approved-cost/count", async (req, res) => {
    try {
      const count = await storage.getApprovedCostRecordCount();
      res.json({ count });
    } catch (error: any) {
  ...
  app.get("/api/approved-cost/meta", async (req, res) => {
    try {
      const meta = await storage.getApprovedCostImportMeta();
      res.json(meta || null);
    } catch (error: any) {
  ...
  app.post("/api/approved-cost/upload", async (req, res) => {
    try {
      const { rows, headers, importedBy } = req.body;
      ...
      await db.delete(approvedCostRecords);
      ...
      const result = await storage.upsertApprovedCostRecords(recordsToUpsert);
      await storage.updateApprovedCostImportMeta(headers, 'COMPOSITE', recordsToUpsert.length, importedBy);
  ...
  app.get("/api/approved-cost/analytics", async (req, res) => {
    try {
  ...
  app.get("/api/registration", async (req, res) => {
    try {
      ...
      const sparesQueryResult = await db.select({ 
      ...
      const localTrucks = await storage.getAllTrucks();
      ...
      const trackingData = await db.select().from(registrationTracking);
  ...
  app.patch("/api/registration/tracking/:truckNumber", async (req, res) => {
    try {
      const { truckNumber } = req.params;
      ...
      // Get existing record
      const existing = await db.select().from(registrationTracking).where(eq(registrationTracking.truckNumber, truckNumber)).limit(1);
  ...
  app.post("/api/registration/import", async (req, res) => {
    try {
      const { data } = req.body;
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (existing.length > 0) {
          ...
          if (!existing[0].registrationRenewalDate) {
            await db.update(spareVehicleDetails)
          ...
          await db.insert(spareVehicleDetails).values({
            vehicleNumber,
  ...
  app.get("/api/decommissioning", async (req, res) => {
    try {
      const vehicles = await storage.getAllDecommissioningVehicles();
  ...
  app.patch("/api/decommissioning/:id", async (req, res) => {
    try {
      const { id } = req.params;
      const updates = req.body;
      
      const vehicle = await storage.updateDecommissioningVehicle(parseInt(id), updates);
      if (!vehicle) {
  ...
  app.delete("/api/decommissioning/:id", async (req, res) => {
    try {
      const { id } = req.params;
      await storage.deleteDecommissioningVehicle(parseInt(id));
      res.json({ success: true });
    } catch (error: any) {
  ...
  app.post("/api/decommissioning/import", async (req, res) => {
    try {
      ...
      const result = await storage.bulkUpsertDecommissioningVehicles(vehicles);
  ...
  async function syncDecommissioningTechData(): Promise<{ synced: number; preserved: number; total: number }> {
    ...
    for (const vehicle of vehicles) {
      ...
      if (snowflakeData) {
        ...
        await storage.updateDecommissioningVehicle(vehicle.id, {
          vin,
    ...
    if (unmatchedWithZip.length > 0) {
      ...
      for (const vehicle of unmatchedWithZip) {
        ...
        if (nearestManager) {
          ...
          await storage.updateDecommissioningVehicle(vehicle.id, updateData);
          zipFallbackSynced++;
  ...
  app.post("/api/decommissioning/sync-tech-data", async (req, res) => {
    try {
  ...
  app.post("/api/decommissioning/sync-from-pos", async (req, res) => {
    try {
      const allPurchaseOrders = await storage.getAllPurchaseOrders();
      const existingDecomVehicles = await storage.getAllDecommissioningVehicles();
  ...
  async function calculateDecommissioningDistances(): Promise<{ managerCalculated: number; techCalculated: number }> {
    ...
    const vehicles = await storage.getDecommissioningVehiclesNeedingDistanceCalc();
    ...
    for (const result of results) {
      ...
      await storage.updateDecommissioningVehicleDistance(
        result.id,
  ...
  app.post("/api/decommissioning/calculate-distances", async (req, res) => {
    try {
  ...
  app.post("/api/decommissioning/sync-parts-count", async (req, res) => {
    try {
  ...
  app.get("/api/reg-messages/:truckNumber", async (req, res) => {
    try {
      const { truckNumber } = req.params;
      const normalized = truckNumber.padStart(6, '0');
  ...
  app.get("/api/reg-conversations", async (req, res) => {
    try {
  ...
  app.post("/api/reg-messages", async (req, res) => {
    try {
      ...
      if (nextAllowed) {
        ...
        await db.insert(regScheduledMessages).values({
      ...
      try {
        ...
        const [failedMsg] = await db.insert(regMessages).values({
          truckNumber: normalized,
          techId: ldap || null,
          techPhone: formattedPhone,
          direction: 'outbound',
      ...
      const [msg] = await db.insert(regMessages).values({
        truckNumber: normalized,
        techId: ldap || null,
        techPhone: formattedPhone,
        direction: 'outbound',
  ...
  app.patch("/api/reg-messages/read/:truckNumber", async (req, res) => {
    try {
      const { truckNumber } = req.params;
      const normalized = truckNumber.padStart(6, '0');
      await db
  ...
  app.post("/api/webhooks/twilio-reg", async (req, res) => {
    try {
