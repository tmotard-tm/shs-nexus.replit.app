...
export class DatabaseStorage implements IStorage {
  ...
  async deleteTruck(id: string): Promise<void> {
    await db.delete(actions).where(eq(actions.truckId, id));
  ...
  async deleteTrackingRecord(id: string): Promise<void> {
    await db.delete(trackingRecords).where(eq(trackingRecords.id, id));
  }
  ...
  async getPmfDataset(): Promise<PmfDataset> {
    ...
    const rows = await db.select().from(pmfRows);
  ...
  async replacePmfData(data: PmfImportData): Promise<PmfImport> {
    ...
    const existingRows = await db.select().from(pmfRows);
    ...
    if (data.rows.length > 0) {
      for (const row of data.rows) {
        ...
        if (existing) {
          ...
          const [inserted] = await db.insert(pmfRows).values({
            importId: newImport.id,
            assetId: row.assetId,
            status: row.status,
            rawRow: JSON.stringify(row.rawRow),
      ...
      if (statusEventsToInsert.length > 0) {
        await db.insert(pmfStatusEvents).values(
      ...
      if (orphanedAssetIds.length > 0) {
        for (const assetId of orphanedAssetIds) {
          await db.delete(pmfRows).where(eq(pmfRows.assetId, assetId));
        }
  ...
  async getPmfStatusEvents(startDate?: Date, endDate?: Date): Promise<PmfStatusEvent[]> {
    let query = db.select().from(pmfStatusEvents).orderBy(desc(pmfStatusEvents.effectiveAt));
  ...
  async backfillPmfStatusEvents(): Promise<{ eventsCreated: number; vehiclesProcessed: number }> {
    ...
    const allRows = await db.select().from(pmfRows);
    ...
    if (eventsToInsert.length > 0) {
      ...
      for (let i = 0; i < eventsToInsert.length; i += batchSize) {
        ...
        await db.insert(pmfStatusEvents).values(batch);
      }
  ...
  async getAllPurchaseOrders(): Promise<PurchaseOrder[]> {
    return await db.select().from(purchaseOrders).orderBy(desc(purchaseOrders.importedAt));
  }
  ...
  async getPoImportMeta(): Promise<PoImportMeta | undefined> {
    const [meta] = await db.select().from(poImportMeta).orderBy(desc(poImportMeta.lastImportedAt)).limit(1);
  ...
  async getArchivedTrucks(): Promise<ArchivedTruck[]> {
    return await db.select().from(archivedTrucks).orderBy(desc(archivedTrucks.archivedAt));
  }
  ...
  async getRentalImports(limit?: number): Promise<RentalImport[]> {
    const query = db.select().from(rentalImports).orderBy(desc(rentalImports.importedAt));
    if (limit) {
  ...
  async upsertPmfActivityLogs(logs: InsertPmfActivityLog[]): Promise<{ inserted: number }> {
    ...
    await db.insert(pmfActivityLogs).values(logs);
  ...
  async clearPmfActivityLogs(vehicleId?: number): Promise<void> {
    if (vehicleId) {
      await db.delete(pmfActivityLogs).where(eq(pmfActivityLogs.vehicleId, vehicleId));
    } else {
      await db.delete(pmfActivityLogs);
    }
  }
  ...
  async getAllDecommissioningVehicles(): Promise<DecommissioningVehicle[]> {
    return await db.select().from(decommissioningVehicles).orderBy(decommissioningVehicles.truckNumber);
  }
  ...
  async deleteDecommissioningVehicle(id: number): Promise<void> {
    await db.delete(decommissioningVehicles).where(eq(decommissioningVehicles.id, id));
  }
  ...
  async getCallLogsByTruckId(truckId: string): Promise<CallLog[]> {
    return await db.select().from(callLogs).where(eq(callLogs.truckId, truckId)).orderBy(desc(callLogs.callTimestamp));
  }

  async getCallLogByConversationId(conversationId: string): Promise<CallLog | undefined> {
    const [log] = await db.select().from(callLogs).where(eq(callLogs.elevenLabsConversationId, conversationId));
    return log;
  }

  async updateCallLog(id: number, data: Partial<CallLog>): Promise<CallLog> {
    const [log] = await db.update(callLogs).set(data).where(eq(callLogs.id, id)).returning();
    return log;
  }

  async getCallLogsByBatchId(batchId: string): Promise<CallLog[]> {
    return await db.select().from(callLogs).where(eq(callLogs.batchId, batchId)).orderBy(desc(callLogs.callTimestamp));
  }

  async getPendingFollowUps(): Promise<CallLog[]> {
    const today = new Date().toISOString().split("T")[0];
    return await db.select().from(callLogs).where(
      and(
  ...
  async getRecentCallLogs(limit: number = 100): Promise<CallLog[]> {
    return await db.select().from(callLogs).orderBy(desc(callLogs.callTimestamp)).limit(limit);
  }
}
