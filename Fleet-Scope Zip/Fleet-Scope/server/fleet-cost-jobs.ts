...
export async function processJobInBackground(jobId: string): Promise<void> {
  ...
  try {
    ...
    await storage.updateFleetCostImportMeta(headers, "ROW_NUMBER", 0, job.importedBy);
    ...
    for (let startRow = range.s.r + 1; startRow <= range.e.r; startRow += CHUNK_SIZE) {
      ...
      if (chunkData.length > 0) {
        const result = await storage.upsertFleetCostRecords(chunkData);
    ...
    await storage.updateFleetCostImportMeta(headers, "ROW_NUMBER", totalInserted + totalUpdated, job.importedBy);
