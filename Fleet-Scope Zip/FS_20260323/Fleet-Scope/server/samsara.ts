...
import { db } from "./db";
import { samsaraLocations } from "@shared/schema";
...
async function loadPersistedSamsaraLocations(): Promise<Map<string, SamsaraLocationData>> {
  ...
  try {
    const results = await db.select().from(samsaraLocations);
...
async function fetchSamsaraLocationsFromApi(): Promise<Map<string, SamsaraLocationData>> {
  ...
  if (!apiToken) {
    throw new Error('SAMSARA_API_TOKEN not configured');
  }
  
  console.log('[Samsara API] Fetching vehicle locations from Samsara API...');
...
export async function testSamsaraApiConnection(): Promise<{ success: boolean; message: string; vehicleCount?: number }> {
  ...
  if (!apiToken) {
    return { success: false, message: 'SAMSARA_API_TOKEN not configured in environment' };
  }
