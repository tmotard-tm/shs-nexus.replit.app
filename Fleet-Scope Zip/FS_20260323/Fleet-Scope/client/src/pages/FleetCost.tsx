...
export default function FleetCost() {
    ...
    while (polls < maxPolls) {
      try {
        ...
        if (job.status === 'completed') {
          ...
          queryClient.invalidateQueries({ queryKey: ["/api/fleet-cost/records"] });
          queryClient.invalidateQueries({ queryKey: ["/api/fleet-cost/analytics"] });
  ...
  const handleApprovedFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setUploadProgress(100);
      queryClient.invalidateQueries({ queryKey: ["/api/approved-cost/analytics"] });
