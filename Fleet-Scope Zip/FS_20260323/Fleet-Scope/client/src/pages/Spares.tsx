...
export default function Spares() {
  ...
  const updateConfirmedAddressMutation = useMutation({
    mutationFn: async ({ vehicleNumber, confirmedAddress }: { vehicleNumber: string; confirmedAddress: string }) => {
      const response = await apiRequest("PATCH", "/api/spares/confirmed-address", { vehicleNumber, confirmedAddress });
      return response.json();
    },
    onMutate: async ({ vehicleNumber, confirmedAddress }) => {
      // Cancel any outgoing refetches to prevent overwriting optimistic update
      await queryClient.cancelQueries({ queryKey: ["/api/spares/locations"] });
      
      // Snapshot the previous value
      const previousData = queryClient.getQueryData(["/api/spares/locations"]);
      
      // Optimistically update the cache
      const normalizedVehicleNumber = vehicleNumber.padStart(6, '0');
      queryClient.setQueryData(["/api/spares/locations"], (old: any) => {
        if (!old) return old;
    ...
    onError: (error: Error, _params, context) => {
      ...
      if (context?.previousData) {
        queryClient.setQueryData(["/api/spares/locations"], context.previousData);
      }
    ...
    onSettled: () => {
      ...
      setTimeout(() => {
        queryClient.invalidateQueries({ queryKey: ["/api/spares/locations"] });
      }, 2000);
    },
  });
  ...
  const updateStatusMutation = useMutation({
    mutationFn: async (params: { 
      ...
      const response = await apiRequest("PATCH", "/api/spares/status", params);
      return response.json();
    },
  ...
  const bulkImportMutation = useMutation({
    mutationFn: async (data: Array<{
      ...
      const response = await apiRequest("POST", "/api/spares/bulk-import", { data });
      return response.json();
    },
    onSuccess: (result) => {
      toast({ 
  ...
  const addManualTruckMutation = useMutation({
    mutationFn: async (data: { truckNumber: string; vin?: string; confirmedAddress?: string }) => {
      const response = await apiRequest("POST", "/api/spares/add-manual", data);
      return response.json();
    },
    onSuccess: (result) => {
