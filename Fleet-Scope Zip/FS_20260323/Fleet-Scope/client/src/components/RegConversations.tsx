...
export function RegConversations({ registrationData, initialTruckNumber }: RegConversationsProps) {
  ...
  const markReadMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/reg-conversations"] });
    },
  });
  ...
  const sendMutation = useMutation({
    mutationFn: (body: string) =>
      apiRequest("POST", "/api/reg-messages", {
    ...
    onSuccess: (data: any) => {
      ...
      queryClient.invalidateQueries({ queryKey: ["/api/reg-messages", selectedTruck] });
      queryClient.invalidateQueries({ queryKey: ["/api/reg-conversations"] });
    },
    onError: (err: any) => {
      toast({
        ...
        try {
          ...
          if (data.type === "reg_message") {
            ...
            if (data.truckNumber === selectedTruckRef.current) {
              queryClient.invalidateQueries({ queryKey: ["/api/reg-messages", selectedTruckRef.current] });
            }
          }
        } catch (e) {
          // ignore parse errors
