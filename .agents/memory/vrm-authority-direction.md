...
- LUCA terminal statuses land VRM-first: append to VRM fleet-status (vocab-validated; its mirror updates fs_trucks with lastUpdatedBy VRM:LUCA), with the legacy direct fs_trucks write kept ONLY as loud fallback on append failure. Every LUCA-touched truck has a VRM case (user-confirmed; case_key = truck-number display form). **Why:** VRM owns rental state; adopt-after-the-fact inverted authority. **How to apply:** any new automation writing FS statuses should go through appendFleetStatus, and follow-up non-status writes must NOT re-stamp lastUpdatedBy — the adopt guard (NOT LIKE 'VRM:%') reads it as "last STATUS writer" (phone-mirror convention). Idempotency = shared already-terminal gate, not dedup rows. Call fields/fs_call_logs still write FS directly by design.
