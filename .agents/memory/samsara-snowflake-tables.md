...
- **SAMSARA_ODOMETER has NO VEHICLE_ID column.** It keys by `NAME` (truck number as Samsara knows it), `SERIAL`, and `VIN`. Any query partitioning/filtering by VEHICLE_ID throws `invalid identifier` — the long-standing `getOdometer()` did exactly this and has been silently erroring for every consumer. Order by `COALESCE(OBD_TIME, GPS_TIME) DESC NULLS LAST`; many rows carry null OBD_TIME and a naive DESC returns them first.
