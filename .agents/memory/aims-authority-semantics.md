...
**Lineage (important):** AIMS_TRUCK_INFO is itself a **nightly batch extract FROM TPMS** (~12:01 AM ET), NOT an independent authority. So the reconciler's "authority" (AIMS `OWNERLDAPID`) and live `/techinfo` are the **same system at two clocks** (nightly-complete vs now) — not two independent confirmations. The Nexus `tpms_tech_profiles` mirror is ALSO TPMS-derived but via the incremental, move-blind `techsupdatedafter` feed — so Nexus holds **two TPMS views that diverge** (AIMS full nightly catches reassignments; the mirror misses them). The genuinely independent confirmations of an assignment are **Holman + AMS**. Heal the mirror by copying AIMS (zero live calls for the bulk); reserve `/techinfo` only for same-day, direct-in-TPMS moves AIMS hasn't captured yet.
