...
import { Search, ChevronDown, ChevronRight, Filter, Calendar, Truck, Wrench, ChevronUp } from "lucide-react";
...
const MOCK_POS = [
  {
    ...
    ataGroup: "33",
    lines: [
      { id: "L1", desc: "Front Brake Pads", ataCode: "33-001", ataGroup: "33 Brakes", repairType: "Replace", amount: 850.00 },
      { id: "L2", desc: "Rotors Machining", ataCode: "33-002", ataGroup: "33 Brakes", repairType: "Machine", amount: 1200.00 },
      { id: "L3", desc: "Labor", ataCode: "33-999", ataGroup: "33 Brakes", repairType: "Labor", amount: 2200.00 },
    ],
    notes: [
      { date: "2026-06-12 09:30", text: "Vendor reported heavy wear on rotors." },
      { date: "2026-06-12 10:15", text: "Approved via automated rules." }
    ]
  },
  {
    id: "PO-90212",
    ...
    ataGroup: "42",
    lines: [
      { id: "L1", desc: "Drive Tires x4", ataCode: "42-004", ataGroup: "42 Tires", repairType: "Replace", amount: 3200.00 },
      { id: "L2", desc: "Mount & Balance", ataCode: "42-999", ataGroup: "42 Tires", repairType: "Labor", amount: 400.00 },
    ],
    notes: [
      { date: "2026-06-10 14:20", text: "Routine replacement at 120k miles." }
    ]
  },
  {
    id: "PO-90215",
    ...
    ataGroup: "01",
    lines: [
      { id: "L1", desc: "Alternator Assembly", ataCode: "01-021", ataGroup: "01 Engine", repairType: "Replace", amount: 1850.00 },
      { id: "L2", desc: "Serpentine Belt", ataCode: "01-022", ataGroup: "01 Engine", repairType: "Replace", amount: 150.00 },
      { id: "L3", desc: "Labor", ataCode: "01-999", ataGroup: "01 Engine", repairType: "Labor", amount: 850.00 },
    ],
    notes: [
      { date: "2026-06-14 08:00", text: "Vehicle towed in, no start condition." }
    ]
  },
  {
    id: "PO-90218",
    ...
    ataGroup: "71",
    lines: [
      { id: "L1", desc: "Mirror Assembly Right", ataCode: "71-045", ataGroup: "71 Body", repairType: "Replace", amount: 1050.00 },
      { id: "L2", desc: "Labor", ataCode: "71-999", ataGroup: "71 Body", repairType: "Labor", amount: 200.00 },
    ],
    notes: [
      { date: "2026-06-08 11:30", text: "Declined: Driver caused damage, pending safety review." }
    ]
  },
  {
    id: "PO-90220",
    ...
    ataGroup: "33",
    lines: [
      { id: "L1", desc: "Rear Brake Adjust", ataCode: "33-005", ataGroup: "33 Brakes", repairType: "Adjust", amount: 400.00 },
      { id: "L2", desc: "Air Valve Swap", ataCode: "33-008", ataGroup: "33 Brakes", repairType: "Replace", amount: 1000.00 },
    ],
    notes: [
      { date: "2026-06-15 10:00", text: "Driver reported air leak on rear axle." }
    ]
  }
];
...
export function AtaBreakdown() {
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-100 font-sans selection:bg-zinc-800 flex flex-col p-4 md:p-6 lg:p-8 space-y-6">
      
      {/* Header & Filter Bar */}
      <div className="flex flex-col space-y-4">
        <h1 className="text-xl font-semibold tracking-tight">ATA Group Breakdown</h1>
        
        <div className="flex flex-wrap items-center gap-3 bg-zinc-900/50 border border-zinc-800/80 p-3 rounded-lg">
          ...
          <div className="flex items-center gap-2 bg-zinc-950 border border-zinc-800 rounded-md px-3 py-1.5 cursor-pointer">
            <span className="text-sm text-zinc-400">Vendor:</span>
          ...
          <div className="flex items-center gap-1 bg-zinc-950 border border-zinc-800 rounded-md p-1">
            <button className="px-3 py-1 text-xs font-medium bg-blue-500/20 text-blue-400 rounded">OPEN</button>
            <button className="px-3 py-1 text-xs font-medium text-zinc-400 hover:text-zinc-300">APPROVED</button>
            <button className="px-3 py-1 text-xs font-medium text-zinc-400 hover:text-zinc-300">CLOSED</button>
            <button className="px-3 py-1 text-xs font-medium text-zinc-400 hover:text-zinc-300">DECLINED</button>
          </div>
            ...
            <div className="space-y-3">
              {ATA_GROUPS.map((group) => {
                ...
                return (
                  <div 
                    key={group.code} 
                    className={`group flex items-center gap-4 cursor-pointer transition-opacity duration-200 ${isFaded ? 'opacity-30' : 'opacity-100'}`}
                    onClick={() => setSelectedAta(isSelected ? null : group.code)}
                  >
                    ...
                    <div className="flex-1 h-6 bg-zinc-950 rounded overflow-hidden relative border border-zinc-800">
                      <div 
                        className={`h-full ${group.color} transition-all duration-500 ease-out`}
                        style={{ width: `${widthPct}%` }}
                      />
                    </div>
                    <div className="w-24 shrink-0 text-sm font-mono text-zinc-300">
              ...
              return (
                <div key={group.code} className="border border-zinc-800 rounded-xl overflow-hidden bg-zinc-900/50">
                  ...
                  <div className={`sticky top-0 z-10 flex items-center justify-between px-4 py-3 border-b border-zinc-800 backdrop-blur-md ${group.bgLight}`}>
                    ...
                    <div className={`font-mono text-sm font-medium ${group.text}`}>
                    ...
                    {posInGroup.map(po => {
                      ...
                      return (
                        <div key={po.id} className="flex flex-col">
                          {/* PO Header Row */}
                          <div 
                            className={`flex items-center gap-4 px-4 py-3 hover:bg-zinc-800/30 cursor-pointer transition-colors ${isExpanded ? 'bg-zinc-800/20' : ''}`}
                          ...
                          >
                            ...
                            <div className="w-24">
                              <span className={`px-2 py-0.5 text-[10px] font-medium uppercase tracking-wider rounded-md border ${TYPE_COLORS[po.type]}`}>
                                {po.type}
                              </span>
                            </div>

                            <div className="w-28">
                              <span className={`px-2 py-0.5 text-[10px] font-medium uppercase tracking-wider rounded-md border ${STATUS_COLORS[po.status]}`}>
                                {po.status}
                              </span>
                            </div>
                            ...
                            <div className="w-24 font-mono text-sm text-right font-medium">{formatCurrency(po.total)}</div>
                          </div>
                                      ...
                                      <tr key={line.id} className="hover:bg-zinc-900/50 transition-colors">
                                        ...
                                        <td className="px-4 py-2">
                                          <span className="inline-flex items-center gap-1.5">
                                            ...
                                            <span className="text-zinc-400 text-xs">{line.ataGroup}</span>
                                          </span>
                                        </td>
                                        <td className="px-4 py-2 text-zinc-400 text-xs">{line.repairType}</td>
                                        <td className="px-4 py-2 font-mono text-right">{formatCurrency(line.amount)}</td>
                                      </tr>
                                    ))}
                                  </tbody>
                                </table>
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4 sticky top-6">
            ...
            <div className="space-y-2">
              {ATA_GROUPS.map(group => (
                <button
                  key={group.code}
                  onClick={() => setSelectedAta(selectedAta === group.code ? null : group.code)}
                  className={`w-full flex items-center justify-between px-3 py-2 rounded-lg border transition-all text-sm
                    ${selectedAta === group.code 
                      ? `${group.bgLight} ${group.border} ${group.text}` 
