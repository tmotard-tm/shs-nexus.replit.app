import {
  ...
  Building2, FileText, LogOut, Moon, Eye, Users, MapPin, FileCode,
} from "lucide-react";

type SubItem = { label: string; icon: typeof Home };
type Category = {
  label: string;
  icon: typeof Home;
  items: SubItem[];
  isNew?: boolean;
  expandedInMockup?: boolean;
};
...
const categories: Category[] = [
  ...
  },
  {
    label: "Queues",
    icon: Clock,
    items: [{ label: "Task Queue", icon: Clock }],
  },
  {
    label: "Management",
    icon: Settings,
  ...
  },
  {
    ...
    icon: Wrench,
    isNew: true,
    expandedInMockup: true,
    items: [
      { label: "All Vehicles",      icon: Truck },
      { label: "Repair Pipeline",   icon: Wrench },
      { label: "Batch Caller",      icon: PhoneCall },
      { label: "Decommissioning",   icon: Archive },
      { label: "Fleet Cost",        icon: DollarSign },
      { label: "PMF Vehicles",      icon: Building2 },
      { label: "SMS Conversations", icon: FileText },
    ],
  },
  {
    label: "Activity",
    icon: Activity,
    items: [{ label: "Audit Log", icon: FileText }],
  },
  {
    label: "Account",
    icon: Key,
    items: [{ label: "API Keys", icon: Key }],
  },
  {
    label: "Help",
    icon: HelpCircle,
    items: [{ label: "Documentation", icon: HelpCircle }],
  },
];
...
export function SidebarMenu() {
  return (
    <div
      className="min-h-screen flex items-start justify-center pt-12 pb-8"
      style={{
    ...
    >
      <div className="relative">
        ...
        <div className="w-10 h-10 rounded-md border border-white/20 bg-white/10 backdrop-blur-sm flex items-center justify-center mb-1.5 cursor-pointer shadow">
          <div className="flex flex-col gap-[3px]">
            <span className="block w-4 h-px bg-white/80 rounded" />
            <span className="block w-4 h-px bg-white/80 rounded" />
            <span className="block w-4 h-px bg-white/80 rounded" />
          </div>
        </div>
            ...
            return (
              <div key={cat.label}>
                ...
                >
                  ...
                  <ChevronRight
                    className={`w-3.5 h-3.5 flex-shrink-0 ${cat.isNew ? "text-amber-400" : "text-zinc-500"}`}
                  />
                </div>

                {/* Fleet Scope — expanded sub-items shown inline */}
                {cat.expandedInMockup && (
                  <div className="bg-zinc-800/50 border-l-2 border-amber-500/50 ml-4 mr-2 mb-1 rounded-r-lg overflow-hidden">
                    {cat.items.map((item) => {
                      const ItemIcon = item.icon;
                      return (
                        <div
                          key={item.label}
                          className="flex items-center gap-3 px-3 py-1.5 text-xs text-amber-200/80 hover:bg-amber-500/10 cursor-pointer transition-colors"
                        >
