...
// Plain TCP Postgres driver (node-postgres); see server/db.ts for why the
// WebSocket-based @neondatabase/serverless driver was removed.
...
fsPool.on("error", (err) => {
  console.error("[fleet-scope-db] idle client error (client will be replaced):", err.message);
});

fsPool.on("connect", (client) => {
  client.query("SET statement_timeout = 15000").catch((err: Error) => {
    console.warn("[fleet-scope-db] could not set statement_timeout:", err.message);
  });
});

export const fsDb = drizzle(fsPool, { schema: fsSchema });
