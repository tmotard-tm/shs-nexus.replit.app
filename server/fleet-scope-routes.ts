...
import { FS_CREATE_INITIAL_STATUS, findChangedOwnedFields, sanitizeCreatePayload, stripOwnedFields } from "./fleet-scope-vrm-guard";
...
async function sendPoDecisionDigestEmail(): Promise<{ sent: boolean; count: number }> {
  if (!process.env.FS_SENDGRID_API_KEY) {
    console.log("[PO Digest] SendGrid API key not configured, skipping digest email");
    return { sent: false, count: 0 };
  }
  // Atomic claim: only one instance/caller can ever own a batch of rows.
  const changes = await fleetScopeStorage.claimUndigestedDecisionChanges();
  if (changes.length === 0) return { sent: false, count: 0 };
  const esc = (v: string | null | undefined) => String(v ?? "").replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
  const fmt = (v: string | null) => (v && v.trim() ? v : "Pending");
  const whenEt = (d: Date | null) => (d ? new Date(d).toLocaleString("en-US", { timeZone: "America/New_York" }) : "");
  const rowsHtml = changes.map(c => `<tr><td style="padding:6px 10px;border:1px solid #ddd;">${esc(c.vehicleNo)}</td><td style="padding:6px 10px;border:1px solid #ddd;">${esc(c.poNumber)}</td><td style="padding:6px 10px;border:1px solid #ddd;">${esc(fmt(c.oldValue))}</td><td style="padding:6px 10px;border:1px solid #ddd;font-weight:bold;">${esc(fmt(c.newValue))}</td><td style="padding:6px 10px;border:1px solid #ddd;white-space:nowrap;">${esc(whenEt(c.changedAt))}</td></tr>`).join("");
  const textLines = changes.map(c => `Truck ${c.vehicleNo || "?"} (row ${c.poNumber || "?"}): ${fmt(c.oldValue)} -> ${fmt(c.newValue)}`).join("\n");
  const msg = {
    to: ["Howard.Anderson@transformco.com"],
    from: "notifications@shs.com",
    subject: `PO Final Approval digest - ${changes.length} decision${changes.length === 1 ? "" : "s"} updated`,
    trackingSettings: { clickTracking: { enable: false, enableText: false } },
    text: `Final Approval decisions updated:\n\n${textLines}\n\nReview at https://fleet-scope.replit.app/fleet-scope/pos`,
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 700px; margin: 0 auto;">
        <h2 style="color: #333;">PO decision digest</h2>
        <p>${changes.length} Final Approval decision${changes.length === 1 ? " was" : "s were"} updated.</p>
        <table style="border-collapse:collapse;font-size:14px;width:100%;">
          <thead><tr>
            <th style="padding:6px 10px;border:1px solid #ddd;background:#f5f5f5;text-align:left;">Truck</th>
            <th style="padding:6px 10px;border:1px solid #ddd;background:#f5f5f5;text-align:left;">Row</th>
            <th style="padding:6px 10px;border:1px solid #ddd;background:#f5f5f5;text-align:left;">Was</th>
            <th style="padding:6px 10px;border:1px solid #ddd;background:#f5f5f5;text-align:left;">Now</th>
            <th style="padding:6px 10px;border:1px solid #ddd;background:#f5f5f5;text-align:left;">When (ET)</th>
          </tr></thead>
          <tbody>${rowsHtml}</tbody>
        </table>
        <p style="margin-top:20px;"><a href="https://fleet-scope.replit.app/fleet-scope/pos" style="color:#0066cc;">Open Purchase Orders</a></p>
        <hr style="border:none;border-top:1px solid #eee;margin:20px 0;">
        <p style="color:#666;font-size:12px;">Automated daily digest from Fleet Scope.</p>
      </div>`,
  };
  sgMail.setApiKey(process.env.FS_SENDGRID_API_KEY);
  try {
    ...
    // Release the claim so the rows are retried on the next trigger.
    await fleetScopeStorage.unclaimDecisionChanges(changes.map(c => c.id)).catch(unclaimErr =>
      console.error("[PO Digest] CRITICAL: failed to unclaim after send failure:", unclaimErr));
    throw sendErr;
  }
  console.log(`[PO Digest] Sent digest with ${changes.length} change(s)`);
...
// applyCallResultToTruck removed with the ElevenLabs caller: LUCA's write-back
// (server/luca-writeback) is the only path that updates fs_trucks call fields
// and fs_call_logs now. Its exhaustive outcome mapping lives in
// server/luca-writeback/mapper.ts.
export function registerFleetScopeRoutes(requireAuth: (req: any, res: any, next: any) => Promise<any>): Router {
  ...
  // Twilio inbound webhooks (all called by external services with their own
  // signature/auth, not a browser session).
  ...
  // Snowflake/external endpoints (rental-ops/open, hr/tech-status,
  ...
  app.get("/trucks", async (req, res) => {
    try {
      ...
      // Holman status facet is intentionally NOT included here.
    ...
    res.json({
      ...
      endpoints: [
        ...
        },
        {
          ...
          description: "Repair-shop call history (written by the LUCA agent). Supports ?truckId=, ?truckNumber=, ?outcome=, ?status=, ?limit= (max 500), ?page=",
          auth: true,
        },
        {
  ...
  app.get("/queue/today", async (_req, res) => {
    try {
      // Builder extracted to server/todays-queue.ts — shared with the VRM
  ...
  app.post("/trucks", async (req, res) => {
    try {
      // VRM owns rental status/call fields: user-facing creates cannot set
      // them. Any supplied owned values are discarded and the truck is
      // force-initialized to the neutral default; VRM's reconcile adopts it
      // as the opening history row once a rental case opens for this truck.
      const { sanitized, discarded } = sanitizeCreatePayload(req.body ?? {});
      const validated = insertTruckSchema.parse(sanitized);
      if (discarded.length > 0) {
        console.log(`[FS Trucks] create ${validated.truckNumber}: discarded VRM-owned field(s) ${discarded.join(", ")}; initialized as '${FS_CREATE_INITIAL_STATUS.mainStatus}'`);
      }
  ...
  const updateTruckHandler = async (req: any, res: any) => {
    try {
      ...
      // VRM is the authority for rental state (2026-08-04): Fleet Scope
      // clients may no longer change these fields — edits happen in VRM
      // Rental Operations and mirror down. A field that arrives UNCHANGED is
      // stripped silently (the Dashboard historically re-sends the current
      // mainStatus to satisfy validation); an actual value change is rejected
      // so the caller learns the edit did not stick. Server-internal writers
      // (LUCA write-back, VRM mirror, van-picked-up)
      // call fleetScopeStorage.updateTruck directly and are unaffected, as
      // are this handler's own derivations below (they run after this guard).
      const changedOwnedFields = findChangedOwnedFields(existing as any, validated as any);
      if (changedOwnedFields.length > 0) {
        return res.status(403).json({
          message: `Rental status and call fields are managed in VRM Rental Operations and mirror to Fleet Scope automatically. Blocked change to: ${changedOwnedFields.join(", ")}.`,
          vrmOwnedFields: changedOwnedFields,
        });
      }
      stripOwnedFields(validated as any);
      ...
      if (validated.registrationStickerValid !== undefined && 
        ...
        // (Removed 2026-08-04) This used to derive subStatus="Ordering duplicate
        // tags" when the sticker state became "Ordered duplicates". Status is
        // owned by VRM Rental Operations — a user-facing FS edit must never
        // originate a status change, even a derived one. Set it in VRM instead.
      }
      ...
      // (Removed 2026-08-04) This used to auto-set mainStatus/subStatus to
      // "On Road"/"Delivered to technician" when vanPickedUp AND
      // spareVanAssignmentInProcess were both true. Status is owned by VRM
      // Rental Operations — the flags themselves remain editable here, but the
      // status transition is made in VRM (see fleet-scope-vrm-guard.ts).
  ...
  // POST /trucks/call-import removed 2026-08-04: call data is owned by VRM/LUCA
  // (fs_call_logs + LUCA write-back). Manual CSV call imports ended when Fleet
  // Scope became a read-only mirror for rental status & call fields.
    ...
    try {
      ...
      for (let i = 0; i < trucks.length; i++) {
        try {
          ...
          // VRM owns rental status/call fields (2026-08-04): CSV status
          // columns are IGNORED — imported trucks are force-initialized to
          // the neutral default and VRM Rental Operations owns status from
          // there (its reconcile adopts the opening value once a case opens).
          const { sanitized, discarded } = sanitizeCreatePayload({
            ...truckData,
            truckNumber: trimmedTruckNumber,
            datePutInRepair: trimmedDate,
          });
          if (discarded.length > 0 || (typeof truckData.status === "string" && truckData.status.trim() !== "")) {
            statusColumnsIgnored++;
          }
          // Remove legacy combined status field from data
          delete (sanitized as Record<string, unknown>).status;
      ...
      res.status(201).json({
        ...results,
        ...(statusColumnsIgnored > 0
          ? {
              statusColumnsIgnored,
              note: `Status columns are ignored — imported trucks start as '${FS_CREATE_INITIAL_STATUS.mainStatus}'; rental status is managed in VRM Rental Operations.`,
            }
          : {}),
      });
    } catch (error: any) {
  ...
  // POST /csv-import removed 2026-08-04: legacy hardcoded 5-row seed endpoint
  // (unreferenced by any client). It created trucks with arbitrary statuses,
  // which VRM Rental Operations now owns — see fleet-scope-vrm-guard.ts.
  ...
  app.patch("/pos/:id/final-approval", async (req, res) => {
    try {
      ...
      const existingPo = await fleetScopeStorage.getPurchaseOrderById(id);
      const previousApproval = existingPo?.finalApproval || "";

      await fleetScopeStorage.updatePurchaseOrderFinalApproval(id, finalApproval);

      // Audit log feeding the daily decision digest (only when the value actually changed)
      if (existingPo && previousApproval.trim() !== finalApproval.trim()) {
        try {
          let vehicleNo = "";
          if (existingPo.rawData) {
            const rd = JSON.parse(existingPo.rawData);
            vehicleNo = String(rd['Vehicle_No'] || rd['Vehicle No'] || rd['VEHICLE_NO'] || rd['Truck #'] || rd['Truck Number'] || rd['TRUCK_NUMBER'] || "");
          }
          await fleetScopeStorage.logPoDecisionChange({
            poId: existingPo.id,
            poNumber: existingPo.poNumber || null,
            vehicleNo: vehicleNo || null,
            oldValue: previousApproval || null,
            newValue: finalApproval || null,
          });
        } catch (logErr) {
          console.error("[PO Digest] Failed to log decision change:", logErr);
        }
      }
  ...
  // Send the daily Final Approval decision digest to Howard Anderson.
  // Idempotent: only changes not yet marked digested_at are included, so external
  // crons (x-internal-cron) and manual calls are always safe to fire.
  app.post("/pos/decision-digest/send", async (_req, res) => {
    try {
      const result = await sendPoDecisionDigestEmail();
      res.json({ success: true, ...result });
    } catch (error: any) {
      console.error("[PO Digest] Error:", error);
      res.status(500).json({ message: "Failed to send decision digest: " + error.message });
    }
  });

  // Best-effort in-process scheduler: send the digest once per ET day after 6 PM.
  // On autoscale an instance may not be awake all evening - the digested_at marker
  // makes the next opportunity (or the cron endpoint above) catch up losslessly.
  let lastDigestDateEt = "";
  if (process.env.NODE_ENV === "production" || process.env.REPLIT_DEPLOYMENT) {
    setInterval(() => {
      ...
      if (etHour >= 18 && lastDigestDateEt !== etDate) {
        sendPoDecisionDigestEmail()
          .then(() => { lastDigestDateEt = etDate; })
          .catch(err => console.error("[PO Digest] Scheduled send failed, will retry in 15 min:", err));
      }
    }, 15 * 60 * 1000);
  }
