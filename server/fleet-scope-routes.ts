...
export function registerFleetScopeRoutes(requireAuth: (req: any, res: any, next: any) => Promise<any>): Router {
  ...
  app.post("/sync-addresses-from-vrm", async (_req, res) => {
    try {
      ...
      // 1. Get all Confirming Status trucks with no repair address
      const allTrucks = await fleetScopeStorage.getAllTrucks();
      const candidates = allTrucks.filter(
        (t) => t.mainStatus === "Confirming Status" && (!t.repairAddress || t.repairAddress.trim() === "")
      );

      if (candidates.length === 0) {
        console.log("[VrmAddressSync] No candidates found.");
        return res.json({ updated: 0, skipped: 0 });
      }

      // 2. Query VRM Full Log for the latest repair_location per stripped truck number
      const vrmRows = await mainDb.execute(sql`
        SELECT DISTINCT ON (REGEXP_REPLACE(trim_van_num, '^0+', ''))
          REGEXP_REPLACE(trim_van_num, '^0+', '') AS stripped_num,
          repair_location,
          repair_phone
        FROM vrm_new_rental_log
        WHERE trim_van_num IS NOT NULL
          AND trim_van_num <> ''
          AND repair_location IS NOT NULL
          AND repair_location <> ''
        ORDER BY REGEXP_REPLACE(trim_van_num, '^0+', ''), date_of_request DESC NULLS LAST
      `);

      // Build lookup map: strippedNum -> { repairLocation, repairPhone }
      const vrmMap = new Map<string, { repairLocation: string; repairPhone: string | null }>();
      for (const row of (vrmRows as any).rows ?? vrmRows) {
        const stripped = String((row as any).stripped_num ?? "").trim();
        if (stripped) {
          vrmMap.set(stripped, {
            repairLocation: String((row as any).repair_location ?? ""),
            repairPhone: (row as any).repair_phone ? String((row as any).repair_phone) : null,
          });
        }
      }

      // 3. Update matching trucks
      let updated = 0;
      let skipped = 0;
      for (const truck of candidates) {
        const stripped = (truck.truckNumber ?? "").replace(/^0+/, "") || (truck.truckNumber ?? "");
        const vrmEntry = vrmMap.get(stripped);
        if (!vrmEntry?.repairLocation) { skipped++; continue; }
        const updates: Record<string, any> = { repairAddress: vrmEntry.repairLocation };
        if (vrmEntry.repairPhone && (!truck.repairPhone || truck.repairPhone.trim() === "")) {
          updates.repairPhone = vrmEntry.repairPhone;
        }
        await fleetScopeStorage.updateTruck(truck.id, updates);
        updated++;
      }

      console.log(`[VrmAddressSync] Done: updated=${updated}, skipped=${skipped}`);
      return res.json({ updated, skipped });
    } catch (error: any) {
      console.error("[VrmAddressSync] Error:", error.message);
      return res.status(500).json({ message: error.message || "VRM address sync failed" });
    }
  });
