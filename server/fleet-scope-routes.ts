...
"status": one of these exact values: "Will Pick Up", "No Answer", "Voicemail", "On Hold", "Other Issues"
...
export async function applyCallResultToTruck(
  ...
  try {
    ...
    } else if (mappedOutcome === "CALL_NO_CONTACT") {
      const d = new Date(today);
    ...
    if (existingLog) {
      ...
      await fleetScopeStorage.updateCallLog(existingLog.id, {
        ...
        attemptNumber: mappedOutcome === "CALL_NO_CONTACT" ? currentAttempt + 1 : currentAttempt,
        nextFollowUpDate: mappedOutcome === "VEHICLE_READY" ? null : (nextFollowUp || existingLog.nextFollowUpDate),
      });
    } else {
