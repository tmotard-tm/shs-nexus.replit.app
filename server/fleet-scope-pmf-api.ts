...
class ParqApiService {
  ...
  private getCredentials(): { clientId: string; clientSecret: string } {
    const clientId = (process.env.FS_PMF_CLIENT_ID || '').trim();
    const clientSecret = (process.env.FS_PMF_CLIENT_SECRET || '').trim();
    
    if (!clientId || !clientSecret) {
      throw new Error('FS_PMF_CLIENT_ID and FS_PMF_CLIENT_SECRET environment variables are required');
    }
