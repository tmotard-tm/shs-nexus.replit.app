...
export default function AllVehicles() {
  ...
  const { data: byovSnapshots } = useQuery<ByovSnapshotsResponse>({
    queryKey: ["/api/fs/byov/weekly-snapshots"],
  });
  ...
  const { data: pickupSnapshots } = useQuery<Array<{ id: string; captured_at: string; week_number: number; week_year: number; pickups_scheduled: number; week_label: string | null; truck_numbers: string[] | null }>>({
    queryKey: ["/api/fs/pickup-weekly-snapshots"],
  });
  ...
  const updateSnapshotMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["/api/fs/pickups-scheduled-this-week"] });
  ...
  const captureSnapshotMutation = useMutation({
    mutationFn: async () => {
      return await apiRequest("POST", "/api/fs/byov/capture-snapshot", { capturedBy: currentUser });
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/fs/byov/weekly-snapshots"] });
