...
| Holman portal | Headless Chromium login (JS-gated) | `holman-portal-service.ts` + `holman-headless-login.ts` | Login runs in an ISOLATED child process (`holman-login-worker.ts`, one-JSON-line stdout contract, 120s group-kill); never launch Chromium in the Express process. Prod MUST have a clean (non-privacy-patched) Chromium in the deploy image — `replit.nix` declares `pkgs.chromium` (stock chromium-125) and `resolveChromiumPath()` prefers it (`-chromium-<digit>` in /nix, ungoogled excluded) over the `ungoogled-chromium` fallback; ungoogled is privacy-patched and its post-login cookie/JS behavior broke the in-page harvest in prod (all builds run `page.evaluate` fine — the old "rev-1080 corrupts evaluate" note was wrong; playwright-core is 1.41.2). `pkgs.playwright-driver.browsers` would be ideal but is a dotted sub-attr the package tool can't install, so stock `pkgs.chromium` is used instead. Approve/deny re-reads the page for confirmation; HTTP 200 is never proof. |
| TPMS | XML token (Basic header), 60-min cache | `tpms-service.ts` | `GET /techinfo/{id}` accepts a TRUCK number too, not only an Enterprise ID (Postman example `/techinfo/023132`; domain owner confirmed); the in-code comment/error string in `tpms-service.ts` claiming "no truck lookup" is WRONG. Current `lookupByTruckNumber()` is deliberately cache-only (`tpms_tech_profiles`), but truck->tech does not have to be. `PUT /techinfo` batch upserts (success = empty messages). `updatetruckdist` is REJECTED while a tech is assigned. District numbers must be zero-padded (`0007088` style); `updatedBy` 6-9 chars. |
