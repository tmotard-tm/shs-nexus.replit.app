...
-   **WO Roster Queue Filtering**: Queue pages (NTAO, Assets, Inventory, Fleet, unified Queue Management) are filtered to only show items for technicians on the current Weekly Offboarding roster. An in-memory cache of roster identifiers (emplIds + enterpriseIds, normalized to uppercase) is populated when `syncTermedTechs` runs or when the `/api/weekly-offboarding` endpoint is called. The `filterQueueItemsByWORoster()` helper cross-references queue item `data.employeeId`/`data.techRacfid` against the cache. Fail-open design: if cache is null (not yet populated), all items are shown. Legacy items from previous offboarding cycles remain in the DB but are hidden from queue views. Add `?showAll=true` to any queue GET endpoint to bypass the filter.
