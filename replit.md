...
-   **Vehicle Number Utility**: Centralized formatting in `shared/vehicle-number-utils.ts` — `toHolmanRef()` (6-digit), `toTpmsRef()` (6-digit), `toDisplayNumber()` (5-digit min), `toCanonical()` (stripped leading zeros), `toSnowflakeRef()` (as-received). All vehicle number padding goes through this module instead of ad-hoc `padStart` calls. The `vehicles` and `holmanVehiclesCache` tables store dedicated reference columns (`holmanVehicleRef`, `tpmsVehicleRef`, `snowflakeVehicleRef`, `vehicleNumberDisplay`).
