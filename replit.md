...
-   **Locks are episode-scoped, never eternal** (Tyler 8/3): `expireStaleShopPhoneLocks()` (scrape-service, runs at the start of every `runDeltaSweep`, even under `VRM_SKIP_HOLMAN_SCRAPE`) clears a lock once NO present case references the truck (as `case_key` or identity-resolved assigned truck) AND both clocks — the case's `dropped_from_feed_at`/`last_seen_at` and `shop_phone_edited_at` — are > 7 days old (grace absorbs ETL feed flicker; fresh edits never insta-expire). Reset restores the scrape pick from stored hist via the shared `pickShopFromEvents()` (same pick `upsertTruck` uses — do not fork), flips `shop_phone_source` to `'scrape'`, keeps `edited_by/at` as audit, and logs a `vrm_rental_operation_actions` row (`action_type='shop_phone_lock_expire'`, actor `system`).

## Right Size Tracker — rate is not compliance (2026-08-03, Tyler directive; reverses 7/30)

An authorized rate at/below the sedan ceiling ($59.75) NO LONGER makes a rental right-sized — the vehicle itself must change: sedan nameplate on the ARI report, or the tech confirming the swap by SMS (DONE/RETURNED remain propose-only). Rate-only replies (regex `rateConfirmed` or LLM `RATE_ONLY`) propose `NEW_REPLY`/review instead of DONE so a human re-engages for the actual swap; swap language still outranks rate talk. The LLM now gets the rental-report Make/Model as context (`loadRentedVehicle` in `server/vrm/rightsize/sync.ts`, truck sources mirror `computeCompliance` identity resolution; the prompt warns the report can lag a swap). KPI semantics: `byRateOnly` pinned 0 (snapshot column `by_rate_only` kept so history lines up), `compliantBy="both"` now means model+SMS (was rate+model). Proof-of-life rule: any inbound from a NON_RESPONDER that yields no verdict proposes `NEW_REPLY` for review, guarded (`proposed_stage IS NULL OR = 'NEW_REPLY'`) so it never downgrades a stronger pending proposal. Cutover impact: 30 rentals (~$1.5k/mo over-sedan spend) moved onto the chase list. `classifier.ts`, `classifier.test.ts`, `pipeline.test.ts` are CRLF files — patch via script, not exact-match editing.
