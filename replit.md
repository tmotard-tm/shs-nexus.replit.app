...
# End-to-End Review (April 2026)

Canonical architecture lock for the Nexus / Fleet Scope / VRM consolidation lives in **`docs/end-to-end-review.md`**. Read that file before changing sync, slideout, vendor-adapter, or rate-limit code.

Locked decisions:
1. **TPMS = system of record** for tech-to-truck assignments (vendor wins on conflict).
2. **SendGrid = single account.** `FS_SENDGRID_API_KEY` and the FS_PG* / FS_DATABASE_URL secrets are deprecated.
3. **NetSuite vehicle data flows through the WMS engine only** (`server/wms-engine-service.ts`). No direct NetSuite calls; **no Snowflake-first for vehicle entities.**
4. **Per-system read-tier policy:**
    - **WMS** (vehicles, assignments) → live + 12h bulk reconcile + client-side hash-diff. `useCaseId` standardised on `"Nexus"` via `WMS_ENGINE_USE_CASE_ID`.
    - **Samsara** (telematics) → Snowflake-first (T1 mirror / T2 webhook / T3 live). Webhook is a cache-invalidation hint, not a primary data path; webhook does NOT trigger T3.
    - **PMF** → live-only (no mirror).
    - **Holman, AMS** → webhook + outbox (vendors authoritative); opt-in tiered reads on Snowflake-mirrored fields.
5. **Provenance everywhere:** every served field carries `source`, `source_tier`, `read_tier`, `last_synced_at`, `freshness_threshold_sec`, `conflict_policy` via the `field_provenance` table. Stale-with-flag is the universal degrade path when the per-vendor token bucket rejects a live call.
6. **New consolidated vehicle slideout:** `client/src/components/vehicle/UniversalVehiclePanel.tsx` (skeleton; supersedes the nine legacy vehicle drawers in 2A.2+). Provenance UI surface: `client/src/components/vehicle/FieldProvenanceBadge.tsx`.
