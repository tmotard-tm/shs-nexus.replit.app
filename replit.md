...
## Right Size Tracker — compliance paths clarified (2026-08-03, Tyler)

Right-sized has three independent paths: (1) **rate** — the tech secured the sedan rate (`rate_authorized` ≤ $59.75), even while keeping the larger rental; (2) **vehicle** — sedan nameplate on the ARI report even at a higher rate (`Rate Authorized` is the reservation basis, not invoiced); (3) **SMS** — the tech confirmed the swap (DONE propose-only; RETURNED never credits). `compliantBy="both"` = rate+model. Rate-matched SMS replies (regex rule or LLM `RATE_ONLY` label) propose DONE for review with a rate-specific reason — the human verifies the rate shows on the report; swap language outranks rate talk. The LLM gets the rental-report Make/Model as context (`loadRentedVehicle` in `server/vrm/rightsize/sync.ts`; the prompt warns the report can lag a swap and never overrides the tech's words). Proof-of-life rule (8/3): any inbound from a NON_RESPONDER that yields no verdict proposes `NEW_REPLY` for review, guarded (`proposed_stage IS NULL OR = 'NEW_REPLY'`) so it never downgrades a stronger pending proposal. `classifier.ts`, `classifier.test.ts`, `pipeline.test.ts` are CRLF files — patch via script, not exact-match editing.
